...
def _axis_get(axis_spec: Any, key: str, default: Any = None) -> Any:
    """
    axis_spec が dataclass / SimpleNamespace / dict どれでも読めるようにする。
    """
...
def build_line_chart(
    ...
    *,
    ...
    """
    chart_spec: chart_config.py の ChartSpec（dataclass想定だが、壊れないように柔軟対応）
    ...
    ax2 = ax.twinx() if getattr(chart_spec, "right_axis", None) else None
    ...
    # 軸ラベル
    left_axis = getattr(chart_spec, "left_axis", None)
    right_axis = getattr(chart_spec, "right_axis", None)

    ax.set_ylabel(_axis_get(left_axis, "label", ""))
    if ax2 is not None:
        ax2.set_ylabel(_axis_get(right_axis, "label", ""))
    ...
    def _apply_axis_scale(_ax, axis_spec):
        ...
        # ymin/ymax が両方ある場合のみ set_ylim（無ければ Matplotlib に任せる）
    ...
    # 秒 → mm:ss 表示
    if _axis_get(left_axis, "formatter", None) == "mmss":
        ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, pos: _format_mmss(x)))
    if ax2 is not None and _axis_get(right_axis, "formatter", None) == "mmss":
    ...
    for i, s in enumerate(getattr(chart_spec, "series", []) or []):
        y_col = getattr(s, "col", None)
        ...
        tgt_ax = ax2 if (ax2 is not None and getattr(s, "axis", "left") == "right") else ax
        color = _pick_color(colors, i)

        tgt_ax.plot(
            x,
            y,
            label=getattr(s, "label", y_col),
            linewidth=getattr(s, "width", None) or 2.2,
            linestyle=getattr(s, "linestyle", None) or "-",
            marker=getattr(s, "marker", None) or "o",
            markersize=getattr(s, "marker_size", None) or 4.8,
            color=color,
        )

        # 最新点
        if show_latest_annotation and bool(getattr(chart_spec, "latest_annotation", False)):
            try:
                valid = y.dropna()
                if len(valid) > 0:
    ...
    if roadmap and getattr(chart_spec, "roadmap_keys", None):
        ...
        for ym, row in (roadmap or {}).items():
            ...
            if pd.isna(dt):
                continue
            xs.append(dt)
            # row が dict じゃない可能性もあるので安全に
            if isinstance(row, dict):
                low_vals.append(row.get(low_key))
                mid_vals.append(row.get(mid_key))
                high_vals.append(row.get(high_key))
            else:
                low_vals.append(getattr(row, low_key, None))
                mid_vals.append(getattr(row, mid_key, None))
                high_vals.append(getattr(row, high_key, None))
    ...
    if plotted_last_points:
        for tgt_ax, x0, y0, s, color in plotted_last_points:
            try:
                # mmss かどうかは「該当軸の formatter 指定」で判定（文字列判定はやめる）
                is_mmss = False
                if tgt_ax is ax and _axis_get(left_axis, "formatter", None) == "mmss":
                    is_mmss = True
                if tgt_ax is ax2 and ax2 is not None and _axis_get(right_axis, "formatter", None) == "mmss":
                    is_mmss = True
