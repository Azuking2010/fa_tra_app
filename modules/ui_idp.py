...
#          Sheetsに記載されたIDP情報を読み込み、目標・マイルール・優先アクションをカード形式で表示する。
...
from modules.ui_components import latest_row
...
def _load_idp_data_cached(_storage, storage_key: str, cache_version: int):
    ...
    return df_profile, df_goals, df_player, df_action
...
def _render_header(st, df_profile: pd.DataFrame) -> None:
    ...
    _render_html(
        ...
        f"""
<div class="idp-hero">
  <div class="idp-hero-title">🗺️ Individual Development Plan</div>
  <div class="idp-hero-sub">
    目標・強み・課題・今やることを確認するページです。
  </div>
...
def render_idp(st, storage) -> None:
    ...
    try:
        with st.spinner("IDPデータを読み込み中..."):
            df_profile, df_goals, df_player, df_action = _load_idp_data_cached(
                storage,
                storage_key,
    ...
    with st.expander("IDPデータを更新する", expanded=False):
